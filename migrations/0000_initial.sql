...
CREATE TABLE `chats` (
  ...
  `notifications_muted` integer DEFAULT 0 NOT NULL,
...
CREATE TABLE `dispatch_chat_state` (
...
CREATE INDEX `dispatch_inputs_chat_state_created_idx` ON `dispatch_inputs` (`chat_id`, `state`, `created_at`);
CREATE INDEX `dispatch_batches_status_available_created_idx` ON `dispatch_batches` (`status`, `available_at`, `created_at`);
CREATE INDEX `dispatch_batches_chat_status_created_idx` ON `dispatch_batches` (`chat_id`, `status`, `created_at`);
CREATE INDEX `dispatch_batches_lease_expires_idx` ON `dispatch_batches` (`lease_expires_at`);
