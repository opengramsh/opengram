...
There are four ways to manage agents manually. If you use [OpenClaw](/docs/agents/openclaw-plugin), there is also an automated option.
...
If your agents are managed by [OpenClaw](/docs/agents/openclaw-plugin), you can skip manual configuration entirely. The plugin's setup wizard detects all available OpenClaw agents and imports them into Opengram automatically:

```bash
openclaw opengram setup
```

The wizard creates the corresponding agent profiles in Opengram for you. To add agents later, just re-run the command — previously linked agents stay selected, and you can pick new ones from the list. See the [OpenClaw Plugin](/docs/agents/openclaw-plugin) docs for full setup instructions.
