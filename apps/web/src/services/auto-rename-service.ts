...
export async function validateAutoRenameKey(
  ...
  try {
      ...
      case 'anthropic': {
        ...
        if (!res.ok) {
          ...
          if (res.status === 401 || res.status === 403) {
            throw validationError(`Authentication failed: invalid API key. (${res.status})`);
          }
          if (res.status === 404) {
            throw validationError(`Model "${modelId}" not found. (${res.status}): ${body}`);
          }
    ...
    throw validationError(
      err instanceof Error ? err.message : `Failed to validate API key: ${String(err)}`,
    );
  }
}
...
export async function maybeAutoRenameChat(chatId: string): Promise<void> {
  try {
    ...
    const chat = db.prepare('SELECT title_source FROM chats WHERE id = ?').get(chatId) as
      | { title_source: string }
      | undefined;
    ...
    const messages = db
      ...
      .all(chatId, MESSAGES_FOR_CONTEXT) as Array<{ role: string; content_final: string | null }>;
