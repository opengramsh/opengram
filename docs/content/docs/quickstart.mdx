...
- **Public base URL** -- the URL where Opengram is accessible (e.g. `https://chat.example.com`). Auto-detects [Tailscale](https://tailscale.com/) if available.
...
- **Systemd service** (Linux) / **launchd service** (macOS) -- whether to install and enable a system service for automatic startup.

The wizard writes its output to `~/.opengram/opengram.config.json` (or `$OPENGRAM_HOME/opengram.config.json`). You can edit the file directly at any time.

## 3. Start the server

If the wizard installed a system service (systemd on Linux or launchd on macOS), Opengram is already running. Otherwise, start it manually:
...
```bash
# Update the URL below to match the one you set for your opengram instance
...
If you use [Openclaw](https://openclaw.ai), you can connect your agents to Opengram with our plugin. If the `openclaw` CLI was detected during `opengram init`, this is already configured -- otherwise, install the plugin manually:
