...
[OpenClaw](https://openclaw.ai) is an open-source AI agent framework for building tool-using, multi-step agents. The **@opengramsh/openclaw-plugin** plugin connects OpenClaw agents to Opengram, letting them send and receive messages, handle interactive requests, and stream responses through the Opengram chat UI.
