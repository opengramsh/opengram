...
export const opengramChatTool = {
    ...
    async execute(_toolCallId, params) {
        ...
        switch (params.action) {
            case "create": {
                const cfg = getConfig();
                const agentId = params.agentId ?? cfg.channels?.opengram?.agents?.[0] ?? "unknown";
                const modelId = params.modelId;
                if (!modelId) {
                    return {
                        content: [{ type: "text", text: "Error: modelId is required (no default model configured). Provide the modelId parameter." }],
            ...
            case "update": {
                if (!params.chatId) {
                    return {
                        content: [{ type: "text", text: "Error: chatId is required for update" }],
