 ...
 */
export function registerOpengramCli(api) {
    api.registerCli((ctx) => {
        ...
        opengram
            ...
            const wizardOpts = {};
            if (opts.baseUrl)
                wizardOpts.baseUrl = opts.baseUrl;
            ...
            const { cfg: nextCfg, shouldRestart } = await runSetupWizard(prompter, ctx.config, wizardOpts);
            ...
            try {
                ...
                const { value: existing } = await readJsonFileWithFallback(allowFromPath, []);
 ...
 */
function createClackPrompter(clack) {
