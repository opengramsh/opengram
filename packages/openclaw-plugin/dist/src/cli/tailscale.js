...
function getTailscaleInfo() {
    try {
        ...
        if (!dnsName && tailscaleIPs.length === 0)
            return undefined;
 ...
 */
function buildCandidateUrls(info) {
    const candidates = [];
    let priority = 0;
    if (info?.dnsName) {
    ...
    const ipv4 = info?.tailscaleIPs?.filter((ip) => !ip.includes(":")) ?? [];
 ...
 * nothing responds.
 */
export async function detectOpengramUrl() {
