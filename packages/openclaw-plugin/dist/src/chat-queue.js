 ...
 */
const latestDispatches = new Map();
/**
...
const supersededDispatches = new Set();
 ...
 */
export function enqueueOrSupersede(chatId, dispatchId, callback, client, log) {
 ...
 */
export function setDispatchCleanup(chatId, dispatchId, cleanup) {
    const active = activeCleanups.get(chatId);
    if (active?.dispatchId === dispatchId) {
        active.cleanup = cleanup;
    }
}
function finishDispatch(chatId, dispatchId) {
    const active = activeCleanups.get(chatId);
    if (active?.dispatchId === dispatchId) {
        activeCleanups.delete(chatId);
    }
...
export function markSuperseded(dispatchId) {
