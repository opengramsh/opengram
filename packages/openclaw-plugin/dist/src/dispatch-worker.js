...
import { processClaimedDispatchBatch } from "./inbound.js";
...
function buildWorkerId(explicitWorkerId) {
...
function isLeaseOwnershipError(error) {
...
async function processClaimedBatch(args) {
    const { client, batch, workerId, cfg, log, abortSignal, dispatch, leaseMs, heartbeatIntervalMs, } = args;
    let leaseLost = false;
...
export function startDispatchWorker(params) {
    ...
    const heartbeatIntervalMs = Math.max(250, params.heartbeatIntervalMs ?? DEFAULT_WORKER_HEARTBEAT_INTERVAL_MS);
    ...
    const minConcurrency = Math.max(1, params.minConcurrency ?? DEFAULT_WORKER_MIN_CONCURRENCY);
    const maxConcurrency = Math.max(minConcurrency, params.maxConcurrency ?? DEFAULT_WORKER_MAX_CONCURRENCY);
    const scaleCooldownMs = Math.max(0, params.scaleCooldownMs ?? DEFAULT_WORKER_SCALE_COOLDOWN_MS);
    const claimManyLimit = Math.max(1, params.claimManyLimit ?? DEFAULT_WORKER_CLAIM_MANY_LIMIT);
    return new Promise((resolve) => {
        ...
        let lastScaleAt = 0;
        let saturationSince = null;
        let lastWorkAt = Date.now();
        const maybeAdjustScale = (backlogLikely) => {
            if (!autoscaleEnabled) {
                return;
            }
            const now = Date.now();
            ...
            if (!inCooldown &&
                ...
                const next = Math.min(maxConcurrency, desiredConcurrency + SCALE_UP_STEP);
                if (next !== desiredConcurrency) {
                    desiredConcurrency = next;
                    lastScaleAt = now;
                    saturationSince = null;
                    log?.info(`[opengram] dispatch autoscale up: desiredConcurrency=${desiredConcurrency}`);
                    return;
                }
            }
            if (!inCooldown &&
                ...
                const next = Math.max(minConcurrency, desiredConcurrency - SCALE_DOWN_STEP);
                if (next !== desiredConcurrency) {
                    desiredConcurrency = next;
                    lastScaleAt = now;
                    log?.info(`[opengram] dispatch autoscale down: desiredConcurrency=${desiredConcurrency}`);
                }
            }
        };
        const runPump = async () => {
                ...
                try {
                    ...
                    for (const batch of batches) {
                        ...
                        void processClaimedBatch({
                            ...
                            .catch((error) => {
                            log?.warn(`[opengram] dispatch task error for batch ${batch.batchId}: ${String(error)}`);
                        })
                            .finally(() => {
        ...
        void runPump();
        log?.info(`[opengram] dispatch worker initialized: desiredConcurrency=${desiredConcurrency}, min=${minConcurrency}, max=${maxConcurrency}, autoscale=${autoscaleEnabled}, claimManyLimit=${claimManyLimit}`);
    });
}
