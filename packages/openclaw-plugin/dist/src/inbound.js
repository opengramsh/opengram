...
import { ChatBatchCoordinator } from "./chat-batch-coordinator.js";
...
function startTypingHeartbeat(client, chatId, agentId) {
...
function isEphemeralEventType(eventType) {
    return eventType === "chat.typing" || eventType === "chat.user_typing" || eventType === "message.streaming.chunk";
}
function wait(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
function buildBatchedContent(messages, attachmentNamesByMessageId) {
...
function sanitizeFileNameForTempPath(name) {
 ...
 */
export function startInboundListener(params) {
    const { client, cfg, log, abortSignal, reconnectDelayMs, dispatch } = params;
    let reconnectTimer = null;
    batchCoordinator = new ChatBatchCoordinator(async (chatId, messages) => {
 ...
 */
async function checkDmPolicy(senderId, chatId, cfg, client, log) {
...
async function handleMessageCreated(data, cfg, client, log) {
    ...
    const senderId = payload.senderId ?? "user:primary";
    ...
    const rawMediaIds = (() => {
        if (Array.isArray(trace?.mediaIds)) {
            return trace.mediaIds.filter((id) => typeof id === "string");
        }
...
async function processMessageBatch(args) {
    ...
    const images = collectedImages.length > 0 ? collectedImages : undefined;
...
function buildCompiledContentFromClaimedBatch(batch) {
...
export async function processClaimedDispatchBatch(args) {
    ...
    const senderIds = new Set(batch.items
        .map((item) => item.senderId)
        .filter((senderId) => typeof senderId === "string" && senderId.trim().length > 0));
...
async function handleRequestResolved(data, cfg, client, log, dispatch) {
    const payload = data.payload;
...
function normalizeAgentIdForSessionKey(agentId) {
 ...
 * See KAI-232.
 */
function buildSessionKey(chatId, agentId) {
...
async function dispatchViaSdk(opts) {
    ...
    log?.info(`[opengram] dispatch route: chatId=${chatId} routeAgent=${route.agentId} selectedAgent=${agentId} matchedBy=${route.matchedBy} sessionKey=${sessionKey}`);
    ...
    await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
        ...
        dispatcherOptions: {
            ...
            deliver: async (payload, info) => {
                log?.info(`[opengram] deliver called: kind=${info.kind} textLen=${payload.text?.length ?? 0} hasMedia=${Boolean(payload.mediaUrl)}`);
                await deliver({ text: payload.text, mediaUrl: payload.mediaUrl }, { kind: info.kind });
            },
            onSkip: (payload, info) => {
                skippedReason = info.reason;
                log?.warn(`[opengram] deliver skipped: kind=${info.kind} reason=${info.reason} textLen=${payload.text?.length ?? 0} hasMedia=${Boolean(payload.mediaUrl)}`);
            },
            onError: (err, info) => {
 ...
 */
function buildDeliver(client, chatId, agentId, dispatchId, cfg, log) {
    return async (replyPayload, { kind }) => {
        log?.info(`[opengram] buildDeliver called: kind=${kind} textLen=${replyPayload.text?.length ?? 0} hasMedia=${Boolean(replyPayload.mediaUrl)}`);
        ...
        if (kind === "final") {
            const normalizedFinalText = typeof replyPayload.text === "string" && replyPayload.text.trim()
                ? replyPayload.text
                : undefined;
...
function parseChatId(rawChatId, eventType, log) {
...
export function clearProcessedIdsForTests() {
