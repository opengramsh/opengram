...
export const opengramMediaTool: AgentTool = {
  ...
  description:
    "Upload one or more files to an OpenGram chat (images, audio, PDFs, etc.). " +
  ...
  parameters: Type.Object({
    ...
    filePaths: Type.Optional(
      Type.Array(Type.String(), {
        minItems: 1,
        description:
          "Array of local file paths to upload. Use MediaPaths from the user's inbound context, " +
    ...
    messageId: Type.Optional(
      Type.String({ description: "Link to an existing message. If omitted, a new message is created automatically." }),
    ),
    caption: Type.Optional(
      Type.String({ description: "Optional caption text for the message (defaults to empty)." }),
    ),
  }),
  ...
  async execute(_toolCallId, params) {
    ...
    const results: { filename: string; contentType: string; mediaId: string }[] = [];
