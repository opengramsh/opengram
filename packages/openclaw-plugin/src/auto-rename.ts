...
export async function maybeAutoRename(params: {
  ...
  try {
    ...
    if (chat.title_source !== "default") return;

    // Fetch recent messages to build context.
    const messages = await client.getMessages(chatId, { limit: MESSAGES_FOR_CONTEXT });
    const userAndAgentMessages = messages.filter(
      (m) => m.role === "user" || m.role === "agent",
    );
    if (userAndAgentMessages.length === 0) return;

    const conversationText = userAndAgentMessages
      .map((m) => {
        const role = m.role === "user" ? "User" : "Assistant";
        const text = ((m as Record<string, unknown>).content_final as string | null | undefined)?.trim() ?? "";
    ...
    const userMessageLength = userAndAgentMessages
      ...
      .reduce((sum, m) => {
        const text = ((m as Record<string, unknown>).content_final as string | null | undefined) ?? "";
        return sum + text.trim().length;
      }, 0);
    if (userMessageLength < 30) return;
