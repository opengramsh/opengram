...
export function messageText(message: Message) {
  if (message.content_final?.trim()) return message.content_final;
  if (message.content_partial?.trim()) return message.content_partial;
  if (message.stream_state === 'streaming') return 'Streaming...';
  return '';
}
